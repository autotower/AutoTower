...
class Movielens1mDataset(Dataset):
    ...
    def _read(self):
        if not self._check_preprocess_file() or self._repreprocess:
            ...
            self._split_dataset()
        super(Movielens1mDataset, self)._read()
    ...
    def _transform(self, *args, **kwargs):
        ...
        data['age'] = lbe.fit_transform(data['age'])
...
if __name__ == '__main__':
    ...
    save_file = os.path.join(save_dir, 'log_dataset_ml1m.txt')
    LOGGER = get_logger(AUTOTOWER, save_file)
    data_path = '/data/autotower/datasets/autotower/ml1m/ml1m.csv'
    dataset = Movielens1mDataset(data_path, negative_sample=0, repreprocess=True, logger=LOGGER)
